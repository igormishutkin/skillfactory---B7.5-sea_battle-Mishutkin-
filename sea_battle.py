...
class Field:
    ...
    def add_ship(self, ship):    #  Размещение кораблей (учитывая, что точка не занята, а если это так, то срабатывает исключение)
        ...
        for d in ship.fields:
            for dx, dy in near:
                ...
                if not(self.out(cur)) and cur not in self.busy:
                    if verb:
                        self.field[cur.x][cur.y] = "."    #  Символ ввиде точки, показывающий пользователю, что точка занята
...
class Game:
    ...
    def random_place(self):
        lens = [3, 2, 2, 1, 1, 1, 1]    #  На каждой доске (у ИИ и у игрока) должно находится следующее количество кораблей: 1 корабль на 3 клетки, 2 корабля на 2 клетки, 4 корабля на одну клетку.
    ...
    def greet(self):
        ...
        print("Формат ввода координат числовой: x - отвечает за горизонталь, y - отвечает за вертикаль. Координаты вводятся через пробел.")
